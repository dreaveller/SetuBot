...
public class ImageUtils {
    ...
    public static String downloadImage(String imageUrl) throws IOException {
        return downloadImage(imageUrl, ImageConst.DEFAULT_IMAGE_SAVE_PATH, null);
    }
     ...
     * @param header   http请求header
     * @param imageUrl 网络图片url
     * @param localDir 本地储存地址
     * @param fileName 文件名称，带后缀的那种，如果为空则取链接最后一段作为文件名
     * @param proxy    代理
     * @return 本地图片相对路径
     * @throws IOException 异常
     */
    public static String downloadImage(HashMap<String, String> header, String imageUrl, String localDir, String fileName, Proxy proxy) throws IOException {
        ...
        if (imageUrl.startsWith("https")) {
            conn = HttpsUtils.getHttpsURLConnection(imageUrl, HttpUtils.REQUEST_METHOD_GET, header, proxy);
        } else {
            conn = HttpUtils.getHttpURLConnection(imageUrl, HttpUtils.REQUEST_METHOD_GET, proxy);
        }
...
//        sourceImg.getHeight();
        ...
        //获取图片后缀,不要"."
        String imgType = FileUtils.getFileSuffix(imagePath).replace(".", "");
