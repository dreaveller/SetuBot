...
//        httpURLConnection.getErrorStream();
     ...
     * @return 是否可用
     */
    public static boolean checkPort(String address, int port) {
    ...
    public static Proxy getProxy(int port) {
        return getProxy("127.0.0.1", port);
    }

    public static Proxy getProxy(String address, int port) {
        return getProxy(Proxy.Type.SOCKS, address, port);
    }

    public static Proxy getProxy(Proxy.Type type, String address, int port) {
        Proxy proxy = null;
        if (HttpUtils.checkPort(address, port)) {
